...
namespace numerical
{
  ...
  real adaptive_cav_simp_MPI_composite::solve()
  {
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size); //just to use cout for rank 0
    ...
    if(I.get_inverted() && rank == 0)    //interval control
    {
    ...
    while(I.get_alpha() != b)       //we stop when alpha (begin of dynamical interval) is equal to b (end of fixed interval)
    {
      ...
      real value1 = numerical::cavalieri_simpson_MPI(f, alpha, beta, n);      //compute the integral (with n)
      real value2 = numerical::cavalieri_simpson_MPI(f, alpha, beta, 2*n);    //compute the integral (with 2*n) for error control
  ...
  bool adaptive_cav_simp_MPI_composite::tol_control(const real value1, const real value2) const
  {
    return (std::abs(value1-value2)*2*I.get_length()/15 < tol*(I.get_beta()-I.get_alpha()));
  }

    // use all the interval_helper functions
  void adaptive_cav_simp_MPI_composite::divide_interval_adapt(void)
  {
    I.divide_interval();
  }


  void adaptive_cav_simp_MPI_composite::extremes_update_adapt(void)
  {
    I.extremes_update();
  }


  bool adaptive_cav_simp_MPI_composite::finish_computation() const
  {
    return I.get_finish_flag();
  }


  void adaptive_cav_simp_MPI_composite::print_intervals() const
  {
